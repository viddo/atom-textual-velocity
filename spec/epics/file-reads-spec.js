...
describe('epics/file-reads', () => {
  ...
  afterEach(function () {
    ...
    expect(store.getActions()).toEqual([finalAction], 'should not have any other actions dispatched anymore')
  })
  ...
  describe('when initialScanDone action dispatches', function () {
    ...
    it('should dispatch a initialScanRawFilesRead action', function () {
      ...
      const action = store.getActions().find(action => action.type === expectedAction.type)
      expect(action).toEqual(expectedAction)
    })

    it('should not read a file that have not changed', function () {
      const action = store.getActions().find(action => action.filename === 'nothing-changed.txt')
      expect(action).toBe(undefined)
    })

    it('should read file missing value matching file reader', function () {
      const action = store.getActions().find(action => action.filename === 'not-changed-but-missing-file-reader-value.txt')
    ...
    it('should read new files', function () {
      let action
      action = store.getActions().find(action => action.filename === 'alice.txt')
      expect(action).toBeDefined()
    ...
    function sharedFileSpecs (action: Action) {
      ...
      describe('when initial scan is done', function () {
        ...
        it('should dispatch a file-read action', function () {
          ...
          expect(lastAction).toEqual(A.fileRead({
            filename: 'alice.txt',
            notePropName: 'content',
