...
describe('epics/hidden-columns', () => {
  ...
  beforeEach(() => {
    ...
    contextMenuItems = () => atom.contextMenu.add.calls.slice(-1)[0].args[0]['.textual-velocity .header']
    contextMenuItemsLabels = () => contextMenuItems().map(item => item['label'])
  ...
  describe('when toggling a column command', function () {
    it('should update hidden columns and labels', function () {
      store.clearActions()
      atom.commands.dispatch(workspaceElement, 'textual-velocity:toggle-summary-column')
      expect(store.getActions()[0]).toEqual(A.changeHiddenColumns(['summary']))
      expect(contextMenuItemsLabels()).toEqual(['    Summary', '✓ File type'])

      store.clearActions()
      atom.commands.dispatch(workspaceElement, 'textual-velocity:toggle-file-type-column')
      expect(store.getActions()[0]).toEqual(A.changeHiddenColumns(['summary', 'file-type']))
      expect(contextMenuItemsLabels()).toEqual(['    Summary', '    File type'])

      store.clearActions()
      atom.commands.dispatch(workspaceElement, 'textual-velocity:toggle-summary-column')
      expect(store.getActions()[0]).toEqual(A.changeHiddenColumns(['file-type']))
      expect(contextMenuItemsLabels()).toEqual(['✓ Summary', '    File type'])
    })
  })
})
