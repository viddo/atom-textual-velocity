  ...
  describe('.cellContent', function () {
    ...
    describe('when there is no query applied', function () {
      ...
      it('should return a kind of AST from which a DOM can be created', function () {
        ...
        expect(cellContent[1]).toEqual({attrs: {className: 'text-subtle'}, content: '.md'})
        expect(cellContent[2]).toEqual(jasmine.any(String), 'should be a separator between title and note content preview')
        expect(cellContent[3]).toEqual(jasmine.any(Object), 'note content preview')
    ...
    describe('when there is a search query', function () {
      ...
      it('should provide what parts of title and note content preview that should be highlighted', function () {
        ...
        cellContent = column.cellContent({
          note: note,
          path: path,
          searchMatch: new SearchMatch(/[kƘƙꝀꝁḰḱǨǩḲḳḴḵκϰ₭][dĎďḊḋḐḑḌḍḒḓḎḏĐđD̦d̦ƉɖƊɗƋƌᵭᶁᶑȡᴅＤｄð]/i)
        }) // as generated by sifter.js
        expect(cellContent).toEqual(jasmine.any(Array))
        expect(cellContent[0]).toEqual(jasmine.any(Array), 'title w/ highlight')
        ...
        expect(cellContent[0]).toEqual(['', {attrs: {className: 'text-highlight'}, content: 'ma'}, 'rkdown'])
        expect(cellContent[1]).toEqual({attrs: {className: 'text-subtle'}, content: '.md'})
        ...
        cellContent = column.cellContent({
          note: note,
          path: path,
          searchMatch: new SearchMatch(/[wẂẃẀẁŴŵẄẅẆẇẈẉ][nŃńǸǹŇňÑñṄṅŅņṆṇṊṋṈṉN̈n̈ƝɲȠƞᵰᶇɳȵɴＮｎŊŋ]/i)
        })
        expect(cellContent[0]).toEqual(['markdo', {attrs: {className: 'text-highlight'}, content: 'wn'}, ''])
        expect(cellContent[1]).toEqual({attrs: {className: 'text-subtle'}, content: '.md'})
        ...
        expect(cellContent[1]).toEqual({attrs: {className: 'text-subtle'}, content: '.md'}, 'match of end should only contain rest+end')
        expect(cellContent[3].content).toEqual(
