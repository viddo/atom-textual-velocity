...
class Projects {
  ...
  _createItemsProp() {
    let {openStream, closeStream} = atoms.projectsPaths();
  ...
  _createWatchPathTask(path) {
    this._tasks[path] = task = new Task(require.resolve('./watch-project-task.js'));
    task.projectPath = path; // projectPath to match src/atom/watch-project-task definition
    task.start(path, atom.config.get('core.ignoredNames'), atom.config.get('core.excludeVcsIgnoredPaths'));
    return task;
  }

  _destroyWatchPathTask(path) {
    task = this._tasks[path];
    task.send('dispose');
    return task;
  }
