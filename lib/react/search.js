...
export default class Search extends React.Component {
  ...
  constructor (props: SearchProps) {
    ...
    privates.set(this, {
      ...
      focusInputUnlessTextEditorIsActive: () => {
        ...
        if (IS_INPUT_ELEMENT_REGEX.test(tagName)) {
          focusInput()
        }
      }
    })
  ...
  componentDidMount () {
    const {disposables, focusInput, onKeyDown, focusInputUnlessTextEditorIsActive} = privates.get(this)
    ...
    ReactDOM.findDOMNode(this).addEventListener('keydown', onKeyDown)
  }

  componentDidUpdate () {
    privates.get(this).focusInputUnlessTextEditorIsActive()
  }

  componentWillUnmount () {
    const {disposables, onKeyDown, focusInputUnlessTextEditorIsActive} = privates.get(this)
    focusInputUnlessTextEditorIsActive.cancel()
    disposables.dispose()
    ReactDOM.findDOMNode(this).removeEventListener('keydown', onKeyDown)
  }
