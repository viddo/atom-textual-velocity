...
export default React.createClass({
      ...
      columns: [
        {
          title: 'Name',
          width: 60,
          createCell: item => <Summary path={item.path} preview={item.preview} />
        }, {
          title: 'Date modified',
          width: 20,
          createCell: item => <DateTime date={item.stat.mtime} />
        }, {
          title: 'Date created',
          width: 20,
          createCell: item => <DateTime date={item.stat.birthtime} />
        }
      ],
      visibleItems: [],
      reverseStripes: true,
  ...
  componentWillMount () {
    ...
    const calcOffset = R.curry(val => (val / this.state.rowHeight) | 0)

    this.onStreamsValues(
      bodyHeightProp, scrollTopProp, searchProp, selectedItemProp, this.props.itemsProp,
      (bodyHeight, scrollTop, searchStr, selectedItem, items) => {
        let visibleBegin = calcOffset(scrollTop)
        let visibleEnd = visibleBegin + calcOffset(bodyHeight) + 2 // add 2 to avoid scroll flicker
        this.setState({
