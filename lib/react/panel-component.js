...
export default React.createClass({
  ...
  propTypes: {
    ...
    selectedPathStream: React.PropTypes.object,
    selectPathBus: React.PropTypes.object,
  ...
  componentWillMount () {
    ...
    this.addBaconSideEffect(
      Bacon.combineTemplate({
        ...
        selectedPath: this.props.selectedPathStream
      })
      .onValue(this.setState.bind(this)))
  },
          ...
          <ScrollableContentComponent
            ...
            <table>
              ...
              <tbody className={classNames({'is-reversed-stripes': this.state.paginationOffset % 2 === 1})}>
                {this.state.visibleFiles.map(file => {
                  const isSelected = file.path === this.state.selectedPath
                  return (
                    <tr key={file.aHrTime} className={classNames({'is-selected': isSelected})}
                          onClick={() => { this.props.selectPathBus.push(file.path) }}>
