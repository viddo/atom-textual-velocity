...
export default class Panel {
  ...
  constructor (project) {
    ...
    const focusCmdStream = atoms.createCommandStream('atom-workspace', 'notational:focus-on-search')
    ...
    const reactPanel = ReactDOM.render(
      <PanelComponent focusOnSearchStream={focusOnSearchStream}
        panelHeightStream={atoms.createConfigStream('notational.panelHeight')}
        filesProp={project.filesProp}
        resultsProp={project.resultsProp}
    ...
    this.disposables = new DisposableValues(
      ...
      reactPanel
        ...
        .debounce(500)
        .onValue(h => {
          atom.config.set('notational.panelHeight', h)
        }),
      ...
      behavior.selectedFileProp
        ...
        .onValue(selectedFile => {
          ...
          if (this._previewEditor) {
            this._previewEditor.buffer.setPath(selectedFile.path)
            this._previewEditor.buffer.setText(selectedFile.content)
            this._previewEditor.buffer.clearUndoStack()
          } else {
      ...
      Bacon
        ...
        .onValue(({selectedFile, searchStr}) => {
          if (selectedFile) {
            ...
            // No selection; open as new file
            atom.workspace.open(`${searchStr.trim()}.md`)
          }
        })
    )
  }
