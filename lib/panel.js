  ...
  constructor (repositories) {
    ...
    const reactPanel = React.render(
      <PanelComponent showStream={dblCancelStream.merge(isVisibleStream)}
        ...
        itemsProp={repositories.resultsItemsProp}
        countProp={repositories.resultsCountProp}
      />, this._panelElement)

    const dblResetStream = this._dblTapStream(reactPanel.resetStream)

    const queryProp = Bacon.combineTemplate({
      searchStr: reactPanel.searchProp,
      pageOffset: reactPanel.pageOffsetProp,
      pageSize: reactPanel.pageSizeProp
    })
    ...
    this.disposables.add(
      ...
      queryProp.onValue(q => repositories.query(q))
    )
  }
