...
export default function makeFileWritesEpic (fileWriters: FileWriters) {
  return function fileWritesEpic (action$: rxjs$Observable<Action>, store: Store<State, Action>) {
    return action$
      .filter(action => {
        if (action.type === A.EDIT_CELL_SAVE) {
          ...
          const fileWriter = fileWriters.find(fileWriter => fileWriter.editCellName === editCellName)
          ...
          if (fileWriter && state.selectedNote) {
            ...
            fileWriter.write(path, action.value, (err, result) => {
              if (err) {
                atom.notifications.addError(`Textual Velocity: Failed to save file ${path}:'`, {
                  detail: err.message,
                  stack: err.stack,
                  dismissable: true
                })
