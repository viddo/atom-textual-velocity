...
export default function configChangesEpic (action$: rxjs$Observable<Action>, store: Store<State, Action>) {
  return Observable
    .merge(
      observeConfig('textual-velocity.listHeight').map(A.changeListHeight).distinctUntilChanged(),
      observeConfig('textual-velocity.rowHeight').map(A.changeRowHeight).distinctUntilChanged(),
      observeConfig('textual-velocity.sortDirection').map(A.changeSortDirection).distinctUntilChanged(),
      observeConfig('textual-velocity.sortField').map(A.changeSortField).distinctUntilChanged(),
