...
import {initialScanDone, scannedFile, START_INITIAL_SCAN, DISPOSE} from '../action-creators'

const pathScannerEpic = (action$: rxjs$Observable<ActionType>, store: Store<StoreType, ActionType>) => {
    ...
    .switchMap(() => {
      const add$ = Rx
        ...
        .mergeMap((file: FileType) => {
          const path = Path.join(dir, file.filename)

          return fileFilter.isAccepted(path)
            ? Rx.Observable.of(scannedFile(file))
            : Rx.Observable.empty()
        })
