...
import {DISPOSE, initialScanDone, fileAdded, fileChanged, fileDeleted, START_INITIAL_SCAN} from '../action-creators'
...
export default function pathWatcherEpic (action$: rxjs$Observable<Action>, store: Store<State, Action>) {
  ...
  return Observable
    .merge(
      ...
      Observable.fromEvent(task, 'add').map(filterAndCreateAction(fileAdded)),
      Observable.fromEvent(task, 'change').map(filterAndCreateAction(fileChanged)),
      Observable.fromEvent(task, 'unlink').map(fileDeleted),
      Observable.fromEvent(task, 'ready').mapTo(initialScanDone())
    )
