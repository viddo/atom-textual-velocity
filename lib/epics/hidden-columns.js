...
export default function makeHiddenColumnsEpic (columns: Columns) {
  ...
  return function hiddenColumnsEpic (action$: rxjs$Observable<Action>, store: Store<State, Action>) {
    const disposableCommands = new Disposables(
      ...columns.map(column => {
        const columnName = name(column)
        return atom.commands.add('atom-workspace', `textual-velocity:toggle-${columnName}-column`, () => {
          const hiddenColumns = atom.config.get('textual-velocity.hiddenColumns')
          atom.config.set('textual-velocity.hiddenColumns', hiddenColumns.includes(columnName)
    ...
    return observeConfig('textual-velocity.hiddenColumns')
      .map((hiddenColumns) => {
        ...
        contextMenuDisposable = atom.contextMenu.add({
          '.textual-velocity .header': columns
            .map(column => {
              const columnName = name(column)
              return {
                label: `${hiddenColumns.includes(columnName) ? '   ' : '✓'} ${column.title}`,
