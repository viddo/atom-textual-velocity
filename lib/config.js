...
export const defaultConfig = (cfg => {
  ...
  path: {
    description: `${
      restartExplanation
    }<br/>Path to folder where to find notes. Can be an absolute path or a relative path to \`~/.atom\` (defaults to \`~/.atom/notes\`)`,
    type: "string",
    default: ""
  },
  ignoredNames: {
    ...
    description: `${
      restartExplanation
    }<br/>List of [glob patterns](https://en.wikipedia.org/wiki/Glob_%28programming%29). Files matching these patterns will be ignored, in addition to the ignoredNames defined in core settings`
  },
  excludeVcsIgnoredPaths: {
    ...
    description: `${
      restartExplanation
    }<br/>Files ignored by the the notes path's VCS system will be ignored. For example, projects using Git have these paths defined in the .gitignore file.`
  },
  sortField: {
    default: "name",
    type: "string"
  ...
  concurrentFilesParses: {
    description: `${
      restartExplanation
    }<br/>Defines how many files to parse concurrently when the initial dir scan is done.<br/>${
      concurrentValueConsequenceExplanation
    }`,
    type: "number",
    default: 3,
    minimum: 0
  ...
  concurrentFileReads: {
    description: `${
      restartExplanation
    }<br/>Defines how many concurrent read operations to do per file.<br/>${
      concurrentValueConsequenceExplanation
    }`,
    type: "number",
    default: 3,
