...
const filterAndCreateAction = (
  notesFileFilter: NotesFileFilter,
  actionCreator: (rawFile: RawFile) => Action | void
) => (rawFile: RawFile) => {
  if (notesFileFilter.isAccepted(rawFile)) {
    const { stats } = rawFile;
...
export function makeFilterAndCreateActions(dir: string) {
  ...
  return {
    filterAndCreateAddAction: filterAndCreateAction(
      notesFileFilter,
      A.fileAdded
    ),
    filterAndCreateChangeAction: filterAndCreateAction(
      notesFileFilter,
      A.fileChanged
    )
  };
...
export function initialScanDone(rawFiles: RawFile[]): InitialScanDone {
  return {
