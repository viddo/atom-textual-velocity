...
export default class Interactor {
  ...
  constructor (viewCtrl: ViewCtrlType, pathWatcherFactory: PathWatcherFactoryType, service: ServiceType) {
    ...
    const sifterProp = pathWatcherProp.flatMap(pw => pw.sifterProp).toProperty()
    const initialPathScanDoneStream = pathWatcherProp.flatMap(pw => pw.initialScanDoneProp)
    ...
    this.filesProp = sifterProp.map('.items')

    this.sifterResultProp = Bacon
      .combineTemplate({
        fields: service.fieldsProp,
        query: searchStream,
        sifter: sifterProp.changes().debounceImmediate(100), // avoid spamming updates just because items change, especially on initial scan
