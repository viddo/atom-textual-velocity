...
export default class Interactor {
  ...
  startSession (req) {
    ...
    // Only triggered once so no need to dispose it
    this._session.onInitialResults(({files, sifterResult}) => {
      this._presenter.presentInitialResults({files: files, sifterResult: sifterResult})
    })

    this._disposables.add(
      this._session.onSearchResults(({files, sifterResult}) => {
        this._presenter.presentSearchResults({files: files, sifterResult: sifterResult})
      })
    )
  }
