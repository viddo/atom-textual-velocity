...
import sendMessageTo from './tasks/send-message-to'

// Coordinates all behavior related to querying project folders, into a list of coherent results
class Project {

  constructor () {
    this._task = new Task(require.resolve('.//tasks/paths-watcher.js'))
    this._task.start()
    ...
    this._disposableValues = new DisposableValues(
      ...
      atoms.createOpenProjectStream().onValue(path => {
        sendMessageTo(this._task, 'openProjectPath', {
          path: path,
      ...
      atoms.createCloseProjectStream().onValue(path => {
        sendMessageTo(this._task, 'closeProjectPath', path)
      })
    )
  }
