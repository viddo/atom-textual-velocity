  ...
  constructor () {
    ...
    const newWatchStream = atoms.createOpenProjectPathStream()
      .map(path => {
        ...
        const customPathsCfg = R.defaultTo([], atom.config.get('notational.customPathsCfg'))
        const cfg = R.defaultTo({}, customPathsCfg.find(cfg => cfg.path === path))
        w.pathFinder = new PathFilter(path, {
          excludeVcsIgnores: R.defaultTo(atom.config.get('core.excludeVcsIgnoredPaths'), cfg.excludeVcsIgnoredPaths),
          exclusions: R.defaultTo(atom.config.get('core.ignoredNames'), cfg.exclusions),
          inclusions: R.defaultTo(['*.txt', '*.md'], cfg.inclusions)
        })
