...
export default class Project {
  ...
  constructor () {
    ...
    const newWatchStream = atoms.createOpenProjectStream()
      .map(path => {
    ...
    const closeProjectPathStream = atoms.createCloseProjectStream()
    ...
    const sifterProp = itemsProp
      .debounce(100) // Debounce to avoid creating new Sifter object for each new item during initial scan
    ...
    this.resultsProp = Bacon.when(
        ...
        [closeProjectPathStream, queryResultsProp, queryProp, itemsProp], (_, r, q, items) => createResults(r, q, items)
      )
      .map(r => {
        if (r.regexpStr) {
