...
export default class Project {
  ...
  constructor (rootPath) {
    ...
    const tagsStream = process.platform === 'darwin'
      ? addFilesStream
        .merge(changedFileStream)
        .flatMap(({path}) => {
          let plistBuf
          try {
            plistBuf = xattr.getSync(path, 'com.apple.metadata:kMDItemOMUserTags')
          } catch (err) {
            // e.g. extended attr does not exist
            return
          }
          return Bacon
            .fromNodeCallback(bplist.parseBuffer, plistBuf)
            .map(([tags]) => {
              return {
                path: path,
                tags: tags.join(' ')
