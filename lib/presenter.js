...
export default class Presenter {
  ...
  presentResults ({filter, files, sifterResult}) {
    ...
    this.viewCtrl.displayResults({
      ...
      columns: [
        {title: 'Summary', key: 'title', width: 70, renderCell: row => row.title + ' - ' + row.content.slice(0, 100)},
        {title: 'Created', key: 'created_at', width: 15, renderCell: row => row.created_at},
        {title: 'Updated', key: 'last_updated_at', width: 15, renderCell: row => row.last_updated_at}
      ],
      rows: items
