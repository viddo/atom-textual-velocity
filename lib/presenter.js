  ...
  constructor (int: InteractorType, columnsP: Bacon.Property) {
    ...
    this.rowsS = Bacon
      ...
      .map((searchResult: SearchResultsType) => {
        const {columns, editCellName, notes, notesPath, pagination, selectedPath, sifterResult} = searchResult
        ...
        return sifterResult.items
          ...
          .map((item, i) => {
            ...
            const path = notesPath.fullPath(relPath)
            const rowSelected = path === selectedPath
            const contentParams = {
              note: note,
              path: path,
