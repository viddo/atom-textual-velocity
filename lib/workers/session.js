    ...
    this._filterProp = Bacon
      .update(
        ...
        [this._filterBus.skipDuplicates(R.equals)], (prev, filter) => {
          const newFilter = R.merge(prev, filter) // "If a key exists in both objects, the value from the second object will be used."
          newFilter.strChanged = R.has('str', filter)
          return newFilter
        }
      )
