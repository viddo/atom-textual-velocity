...
export default class NotesCache {
  ...
  constructor () {
    ...
    this._disposableValues = new DisposableValues(
      atom.config.onDidChange('textual-velocity.path', () => {
        this._notes = {}
        atom.notifications.addInfo('Notes path changed!', {
          description: 'Restart session to load notes from the new path'
        })
      }),
  ...
  load (callback: Function) {
    if (atom.enablePersistence) {
      atom.stateStore
        ...
        .then(state => {
          this._notes = R.pathOr({}, [pkg.version, atom.config.get('textual-velocity.path')], state)
  ...
  save (callback: Function) {
    if (atom.enablePersistence) {
      const state = {}
      state[pkg.version] = {}
      state[pkg.version][atom.config.get('textual-velocity.path')] = this._notes
