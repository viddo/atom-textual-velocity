...
export default class ReactView {
  ...
  constructor (panel: Atom$Panel) {
    ...
    this.clickedCellS = this._clickCellBus // only let through a single click on double click
      .map(clickVal => ({
        time: +new Date(),
        clickVal: clickVal}))
      .toProperty({time: 0})
    ...
    this.keyDownS = this._keyDownBus
    this.listHeightS = this._listHeightBus
    this.saveEditedCellContentS = this._saveEditedCellContentBus
    this.scrollTopS = this._scrollTopBus
    this.sortDirectionS = this._sortDirectionBus
    this.sortFieldS = this._sortFieldBus
                ...
                {rows.map(row => {
                  ...
                  return <tr key={row.id}
                    ...
                    {row.cells.map((cell, i) => {
                      const clickVal = {relPath: relPath, cell: cell}
