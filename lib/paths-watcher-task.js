...
export default function (projectPaths, ignoredNames, excludeVcsIgnores) {
  ...
  const itemsProp = Bacon.update(
    ...
    .debounce(20) // make sure to not spam prop changes until changes calms down (e.g. initial scan)

  const sifterProp = itemsProp
    .map(items => new Sifter(items))
    .startWith(new Sifter([]))

  const queryProp = messageReceiver
    .createStream('query')
    .toProperty({
      searchStr: '',
      paginationOffset: 0,
  ...
  const resultsProp = Bacon.combineWith(sifterProp, queryProp, itemsProp, (sifter, q, items) => {
    // see https://github.com/brianreavis/sifter.js/#searchquery-options
    const r = sifter.search(q.searchStr.trim(), {
      fields: ['path', 'content'],
  ...
  const disposableValues = new DisposableValues(
    resultsProp.onValue(r => emit('results', r)),
