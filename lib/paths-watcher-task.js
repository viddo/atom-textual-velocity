...
export default function (projectPaths, ignoredNames, excludeVcsIgnores) {
  ...
  const queryResultsProp = Bacon.combineWith(sifterProp, queryProp, (sifter, {searchStr}) => {
    ...
    return sifter.search(searchStr.trim(), {
      fields: ['path', 'content'],
  ...
  const resultsStream = Bacon.when(
    [queryResultsProp.changes(), queryProp, itemsProp], (r, q, items) => {
      return {
        offset: 0,
        total: r.total,
        items: r.items
          .slice(0, q.paginationSize)
          .map(({id}) => items[id])
      }
    },
    [messageReceiver.createStream('paginateLastQuery'), queryResultsProp, itemsProp], (q, r, items) => {
      return {
