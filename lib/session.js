...
export default class Session {
  async start (dir: string, columns: Columns, fileReaders: FileReaders, fileWriters: FileWriters, noteFields: NoteFields) {
    const panel = atom.workspace.addTopPanel({
      item: document.createElement('div')
    })

    const rootReducer: Reducer<State, Action> = makeRootReducer(columns, fileReaders, noteFields)
    const middlewares = applyMiddleware(thunk, makeEpicMiddleware(columns, fileReaders, fileWriters))
