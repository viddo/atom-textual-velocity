...
export default function selectedNoteReducer (state: ?SelectedNote = null, action: Action, dir: string, nextSifterResult: SifterResult) {
  ...
  switch (action.type) {
    ...
    case A.FILE_DELETED:
      return state && state.filename === action.filename
        ? null
        : state
