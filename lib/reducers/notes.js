...
export default function makeNotesReducer (fileReaders: FileReaders, noteFields: NoteFields) {
    ...
    switch (action.type) {
      case A.INITIAL_SCAN_DONE:
        return initialScan.rawFiles
          .reduce((nextNotes, rawFile) => {
            const note = state[rawFile.filename] || newNote(rawFile)
            nextNotes[rawFile.filename] = noteWithUpdatedFields(note, rawFile.filename)
            return nextNotes
          }, {})
