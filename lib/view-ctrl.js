...
export default class ViewCtrl {
  ...
  constructor (aRenderer) {
    ...
    this._itemIndexClickBus = new Bacon.Bus()
      ...
      this._disposeRenderResults = Bacon
        .combineTemplate({
          ...
          callbacks: {
            ...
            onClickRow: Bacon.constant(index => this._itemIndexClickBus.push(index)),
    ...
    const startProp = Bacon
      .combineTemplate({
        scrollTop: this._scrollTopBus
          .skipDuplicates()
          .merge(this._textInputBus.map(0)) // reset scroll position on search
          .toProperty(0),
        rowHeight: this._rowHeightProp
      })
      .map(paginationStart)
