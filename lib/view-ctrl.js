...
export default class ViewCtrl {
  ...
  @memoize _setupOnceRenderResults () {
    ...
    this._focusTextInputOnChanges()
    this._paginateOnViewChange()
    this._selectItemOnClick()
    this._selectItemOnEditorChange()
    this._previewSelectedPathOnChange()
    this._openSelectedPathOnEnter()
    ...
    this._togglePanelOnCmd()
    this._toggleAtomWinOnCmd()
  ...
  @memoize get _DOMNodeProp () {
    return this._stateProp.map('.DOMNode')
  }

  @memoize get _atomPanelProp () {
    return this._stateProp.map('.panel')
  }

  @memoize get _togglePanelCmdStream () {
    return atoms.createCommandStream('atom-workspace', 'textual-velocity:toggle-panel')
  }

  @memoize get _toggleAtomWinCmdStream () {
    return atoms.createCommandStream('atom-workspace', 'textual-velocity:toggle-atom-window')
  }

  @memoize get _selectedPathProp () {
    return this._resultsBus.map('.selectedPath')
  }
  ...
  _togglePanelOnCmd () {
    this._disposables.add(
      this._atomPanelProp
        .sampledBy(this._togglePanelCmdStream)
        .onValue(atomPanel => {
  ...
  _toggleAtomWinOnCmd () {
    this._disposables.add(
      this._atomPanelProp
        .sampledBy(this._toggleAtomWinCmdStream)
        .onValue(atomPanel => {
  ...
  _focusTextInputOnChanges () {
    this._disposables.add(
      this._DOMNodeProp
        .sampledBy(Bacon
          .mergeAll(
            atoms.createCommandStream('atom-workspace', 'textual-velocity:focus-on-search'),
            this._toggleAtomWinCmdStream,
            this._togglePanelCmdStream
          ))
        .debounce(25)
        .onValue(DOMNode => {
  ...
  _previewSelectedPathOnChange () {
    return this._selectedPathProp
      .debounce(50) // to avoid sluggishness on rapid changes
      .diff(undefined, (oldPath, newPath) => [oldPath, newPath])
  ...
  _openSelectedPathOnEnter () {
    this._disposables.add(Bacon
      .combineTemplate({
        selectedPath: this._selectedPathProp,
        searchStr: this._resultsBus.map('.searchStr')
      })
      .sampledBy(
        this._keyDownBus.filter(R.propEq('keyCode', ENTER))
      )
      .onValue(({selectedPath, searchStr}) => {
        atom.workspace.open(selectedPath || `${searchStr}.md`)
      }))
  }
