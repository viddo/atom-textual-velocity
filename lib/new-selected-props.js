...
export default function ({filesProp, resultsProp, keyDownBus, selectIndexBus, activePathStream, deselectStream}) {
  ...
  const selectedProp = Bacon
    .update(
      ...
      // Explicit path, change directly to item if path is within the notes dir and current search results set
      [activePathStream, filesProp, resultsProp], (current, newPath, files, {items}) => {
